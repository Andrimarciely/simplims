...
from .models import Amostra, Parametro, ParametroAmostra, VisitaTecnica
...
def criar_parametros_para_amostra(sender, instance, created, **kwargs):
    ...
    parametros = Parametro.objects.filter(categoria_parametro__in=categorias).distinct()

    for p in parametros:
        ParametroAmostra.objects.get_or_create(amostra=instance, parametro=p)
